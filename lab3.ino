...
void setup() {
    ...
    // Test case:
    //     Should drive 15cm forward, 15cm to the right, and face to the left.
...
void updateOdometry(char theta_mode) {
    // Calculate distance, bearing, and heading errors.
    d_err = (theta_mode == THETA_MODE_BEARING) ? sqrt(
    ...
    // Calculate the ratio between phi_l and phi_r.
    if (phi_l > phi_r) {
        left_speed_pct = 1;
    ...
    // Determine if wheels should spin backwards.
    flip_left = phi_l < 0;
    flip_right = phi_r < 0;
}
    ...
    switch (current_state) {
        ...
        case CONTROLLER_GOTO_POSITION_PART3:
            ...
            if (d_err < THRESHOLD) {
                ...
                // Stop prioritizing distance and bearing errors.
                at_target = true;
            }

            if (at_target && abs(h_err) < HEADING_THRESHOLD) {
                // We are at the destination and facing the right way.
                // Stop Sparki.
                sparki.moveStop();
                current_state = CONTROLLER_STOP;
                break;
            ...
            // Spins the wheels the calculated amounts.
            sparki.motorRotate(
                MOTOR_LEFT,
